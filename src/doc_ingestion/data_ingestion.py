...
class FaissManager:  # cSpell:ignore Faiss
    ...
    def add_documents(self, docs: List[Document]):
        ...
        for d in docs:
            ...
            self._meta["rows"][key] = True  # Mark as added
            new_docs.append(d)
...
class ChatIngestor:  # cSpell:ignore Ingestor
    ...
    def _split(self, docs: List[Document], chunk_size=1000, chunk_overlap=200) -> List[Document]:
        ...
        self.log.info("Documents split into chunks", chunks=len(chunks), chunk_size=chunk_size, overlap=chunk_overlap)
        return chunks
    ...
    def build_retriever(self,
        ...
        *,
        ...
        try:
            ...
            raise DocumentPortalException("Error in building retriever", e) from e
...
class DocumentComparator:
    ...
    def save_uploaded_files(self, reference_file, actual_file):
        try:
            ...
            for obj, out in ((reference_file, ref_path), (actual_file, act_path)):
            ...
            self.log.info("PDF saved successfully", reference=str(ref_path), actual=str(act_path), session_id=self.session_id)
            return ref_path, act_path
        except Exception as e:
            self.log.error("Failed to save PDF", error=str(e), session=self.session_id)
            raise DocumentPortalException(f"Error in saving PDF files: {str(e)}", e) from e
        ...
        try:
            with fitz.open(pdf_path) as doc:
                ...
                for page_num in range(doc.page_count):
                    ...
                    if text.strip():
                        parts.append(f"\n--- Page {page_num + 1} ---\n{text}")
            self.log.info("PDF read successfully", pdf_path=str(pdf_path), pages=len(parts))
            return "\n".join(parts)
        except Exception as e:
            self.log.error("Failed to read PDF", error=str(e), pdf_path=str(pdf_path))
            raise DocumentPortalException(f"Could not process PDF: {pdf_path}", e) from e
    def combine_documents(self) -> str:
        try:
            ...
            raise DocumentPortalException("Error in combining documents", e) from e
