...
class DocumentIngestor:  # cSpell:ignore Ingestor
    ...
    def __init__(self, temp_dir: str = "data/multi_doc_chat", faiss_dir: str = "faiss_index", session_id: str | None = None):
        try:
            ...
            self.log.info(
                ...
                faiss_base=str(self.faiss_dir),  # cSpell:ignore faiss
                session_id=self.session_id,
                temp_path=str(self.session_temp_dir),
                faiss_path=str(self.session_faiss_dir)
            )
        except Exception as e:
            self.log.error("Failed to _initialize DocumentIngestor", error=str(e))
            raise DocumentPortalException("Initialized error in DocumentIngestor", sys)
    ...
    def ingest_file(self, uploaded_files):
        try:
            ...
            if not documents:
                raise DocumentPortalException("No documents found in the uploaded file", sys)
